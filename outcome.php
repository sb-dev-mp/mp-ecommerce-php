<?php
  ...
  $data = MercadoPago\Payment::find_by_id($id);
  ...
  $statusTitle = "";
  if ($status == 'success')
    $statusTitle = "Compra pagada con éxito";
  if ($status == 'failure')
    $statusTitle = "Hubo un error en el pago";
  if ($status == 'pending')
    $statusTitle = "Tu compra está pendiente de pago";
                  ...
                  <div class="as-searchnav-placeholder" style="height: 77px;">
                        <div class="row as-search-navbar" id="as-search-navbar" style="width: auto;">
                            <div class="as-accessories-filter-tile column large-6 small-3">
                              <h2 class="as-filter-button-text"><?php echo $statusTitle ?></h2>
                            </div>
                        </div>
                    </div>
                      ...
                      <div class="width:60%">
                        <div class="as-producttile-info" style="float:left;min-height: 168px;">
                            <div class="as-producttile-titlepricewraper" style="min-height: 128px;">
                              <?php
                                if ($id != "null")
                                  echo "<h3>ID de transacción: " . $id . "</h3>";
                              ?>
                              <p>External reference: <?php echo $_GET["external_reference"] ?></p>
                              <p>Compra: <strong><?php echo $item->title ?></strong></p>
                              <p>Precio: <strong>$<?php echo $item->unit_price ?></strong></p>
                              <?php
                                if ($id != "null") {
                              ?>
                              <p>Estado de la operación: <?php echo $_GET["collection_status"] ?></p>
                              <p>Tipo de pago: <?php echo $_GET["payment_type"] ?></p>
                              <p>Metodo de pago: <?php echo $data->payment_method_id ?></p>
                              <?php
                                }
                              ?>
                              <?php
                                if ($_GET["collection_status"] == "pending" && $_GET["payment_type"] == "ticket")
                                  echo "<p>Para pagar en <strong>" . $data->payment_method_id . "</strong> usar el id de transacción <strong>" . $id . "</strong></p>";
                                if ($_GET["status"] == "failure")
                                  if ($id != "null")
                                    echo "<p>No pudo ser finalizado el pago. Por favor, vuelva a probar más tarde.</p>";
                                  else
                                    echo "<p>La compra fue cancelada o no pudo ser finalizado el pago. Por favor, vuelva a probar más tarde.</p>";
                              ?>
                            </div>
                        </div>
                      </div>
